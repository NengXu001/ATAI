            ...
            for room in current_rooms:
                ...
                if room['remainingTime'] > 0:
                    ...
                    if not self.chat_state[room_id]['initiated']:
                        ...
                        self.post_message(room_id=room_id, session_token=self.session_token, message='Heyy, welcome to my chatbox. Is there anything you want to know?')
                    ...
                    for message in all_messages:
                        if message['authorAlias'] != self.chat_state[room_id]['my_alias']:
                            ...
                            if message['ordinal'] not in self.chat_state[room_id]['messages']:
                                ...
                                try:
                                    message_= factual.main(question,g).encode('utf-8')
                                except: message_ = "My dream is to become an omniscient robot, but still a lot to learn."
                                self.post_message(room_id=room_id, session_token=self.session_token, message=factual.main(question,g).encode('utf-8'))
...
if __name__ == '__main__':
    username = 'neng.xu_bot'
    password = getpass.getpass('Password of the neng.xu_bot:')
